...
void Board::move(Move *move, bool dontClear) {
...
Board::Board(std::vector<Checker *> *startState, Color turn, std::vector<std::vector<Checker*>>* boardGrid) {
    this->turn = turn;
...
Board *Board::copy() {
    ...
    return new Board(pieces, getTurn(), 0);
}
