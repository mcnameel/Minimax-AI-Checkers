...
int AI_Minimax_07::evaluateBoardState(Board *boardState) {
...
int AI_Minimax_07::earlyGameEval(Board *boardState, std::vector<Checker*> *willBeCaptured) {
...
int AI_Minimax_07::endGameEval(std::vector<Checker *> *myPcs,
    ...
    for (auto &king : myKings) {
        for (auto &wk : otherKings) {
            if(find(willBeCaptured->begin(), willBeCaptured->end(), king) != willBeCaptured->end()) {
                continue;
            }
...
std::vector<Checker *> *AI_Minimax_07::getCapturedPieces(Board *pBoard) {
